...
function regroupement(tab){
    let objet={}
    for(let i=0;i<tab.length;i++){
       let currentname=tab[i].nom
       let currentgroupe=tab[i].groupe
       if (!(currentgroupe in objet)) {
            objet[currentgroupe] = [];
       }
      objet[currentgroupe].push({ nom: currentname });
    }
    return objet
    
}
console.log(JSON.stringify(regroupement(tableau), null, 2));