...
let choix="";//choix dutilisateur si il veut daugmenter la durre


if (mensualité<=(revenuMensuel*0.3)){
    console.log("Prêt accordé. Mensualité : ["+mensualité+"] MAD")
} else if(revenuMensuel>=10000){
    while (mensualité>(revenuMensuel*0.3)) {
       dureeEnMois++;
       mensualité=montantEmprunte/dureeEnMois;
    }
    console.log("on vous propose daugmenter la duree vers "+dureeEnMois+"choisi oui ou non ?")
       if (choix==="oui"){
        console.log("pret accepter")
       }else{
        console.log("pret refuser")
       }    
