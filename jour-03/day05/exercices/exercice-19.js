...
let x=60;
function recherchebinaire(tableau,X){
    let gauche=0;
    let droit= tableau.length-1;
    while (gauche<=droit){
        let milieu=Math.floor((gauche+droit)/2)
        if (tableau[milieu]==X){
            console.log("found it in the index: "+milieu)
            return milieu
